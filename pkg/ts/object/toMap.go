...
// ToMap 结构体转map
// object.ToMap(struct, "map", "")
func ToMap(s any, tag string, methodName string) (res map[string]any, err error) {
	v := getValue(s)
	t := getType(v)
	//
	if t == nil {
	...
	for i := 0; i < t.NumField(); i++ {
		...
		case reflect.Struct:
			...
			// recursive
			deepRes, deepErr := ToMap(fieldValue.Interface(), tag, methodName)
			if deepErr != nil {
				return nil, deepErr
			}
			if flag&flagDive != 0 {
