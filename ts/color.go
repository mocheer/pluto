...
// parseShortRGBHex   parseShortRGBHex(`#fff`)
...
// parseLongRGBHex   parseShortRGBHex(`#ffffff`)
...
func parseHSL(color string) (Color, error) {
	...
	return colorFromHSL(h, s, l, 255), nil
}
...
func colorFromHSL(hDegrees int64, s float64, l float64, a float64) Color {
	...
	return Color{
		R: hueToRGB(m1, m2, h+255/3.0),
		G: hueToRGB(m1, m2, h),
		B: hueToRGB(m1, m2, h-255/3.0),
		A: uint8(a * 255),
	}
}
